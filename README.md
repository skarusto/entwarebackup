...
opkg install curl
```
2. Скопируйте скрипт к себе в Entware
```
...
```
4. Любым встроенным редактором в секции #Telegram settings замените значения "YOUR_BOT_TOKEN" и "YOUR_CHAT_ID" на свои, например
```
nano /opt/backup.sh
```
5. (необязательно) Добавлена секция #Router settings, чтобы можно было задать переменную ROUTER_NAME, которая будет использоваться при генерации имени файла бэкапа.
Замените значение "YOUR_ROUTER_NAME" на своё (напримре, ROUTER_DOM, ROUTER_KVARTIRA)

Актуально для владельцев нескольких роутеров, которые хотят хранить их бэкапы в одной телеграм-группе и различать их.
