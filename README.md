...
nano /opt/backup.sh
```
5. (необязательно) В секции #Router settings замените значение по умолчанию переменной ROUTER_NAME, которая используется при генерации имени файла бэкапа.
Замените значение "Keenetic" на своё (напримрер, Keenetic_Dom, Keenetic_Kvartira).
